...
public class GameRoom extends JFrame {
    ...
    private int p1Pick; //게임 시작 때 확정됨 p2도 마찬가지
    private int p2Pick;
    ...
    private int[][] board = new int[19][19]; // 오목판 상태, 0: 빈 칸, 1: 흑돌, 2: 백돌
    ...
    public GameRoom(String roomNumber, int uid, PrintWriter out) {
        ...
        boardLabel = new BoardLabel(new ImageIcon("src/main/java/img/오목판690.png")); // 오목판 이미지 설정
        boardPanel.add(boardLabel); // 패널에 이미지 추가
        add(boardPanel, BorderLayout.WEST); // 패널을 WEST 영역에 추가
        boardLabel.setBoardClickListener((x, y) -> {
            out.println("GAME:MOVE,"+roomNumber+","+uid + "," + x + "," + y);
        });
        ...
        //기권 버튼~
        giveupLabel=new JLabel("기권하기");
        giveupLabel.setBounds(400, 0, 50, 30);
        giveupLabel.setForeground(new Color(255, 138, 138));
        giveupLabel.setFont(customFont2);
        giveupLabel.setVisible(false);
        userListPanel.add(giveupLabel);
        giveupLabel.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                ...
                if (result == JOptionPane.YES_OPTION) {
                    String message = "GAME:GVUP," + roomNumber + "," + uid;
                    out.println(message);
                }
                boardLabel.clearBoard();
                resetBoard();
            }
        });
        ...
        sendButton.setBackground(new Color(244, 246, 255));
        sendButton.setFont(customFont2);
    ...
    public void gameStart() {
    	chatArea.append("[Notice] 게임이 시작됩니다!\n");
    	String p1Stone = pickPath[p1Pick - 1].getAbsolutePath();
    	String p2Stone = pickPath[p2Pick - 1].getAbsolutePath();
    	ImageIcon p1Icon = new ImageIcon(p1Stone);
    	ImageIcon p2Icon = new ImageIcon(p2Stone);
        player1ReadyButton.setVisible(false);//게임 시작하니까 준비 버튼 숨김
        player2ReadyButton.setVisible(false);
        giveupLabel.setVisible(true);
        // 플레이어 1과 2의 돌 설정 (여기서 다른 UI 업데이트 및 게임 초기화 로직도 처리)
        boardLabel.setIcons(p1Icon, p2Icon);

        // 게임 시작 시 UI 업데이트 필요
    }
    
    public void updateBoard(int x, int y, int stone) {
        if (boardLabel == null) return; // boardLabel이 null이면 반환

        // 플레이어 1(흑돌)과 플레이어 2(백돌)에 맞는 값을 설정
        boardLabel.setStone(x, y, stone);

        // 현재 플레이어가 놓은 돌을 board에 반영
        board[x][y] = stone;
    ...
    public void displayWinner(int winnerPick) {
        String winnerNick = (winnerPick == 1) ? p1Nick : p2Nick;
        String winnerMessage = winnerNick + "님이 승리하셨습니다!";

        // 승리 메시지 팝업 표시 후 확인 버튼 클릭 시 실행될 코드
    ...
    public void endGame() {
    	player1ReadyButton.setText("준비"); // "준비" 텍스트로 초기화
    	player1ReadyButton.setBackground(new Color(244, 246, 255)); // 초기 색상으로 되돌림
    	player2ReadyButton.setText("준비"); // "준비" 텍스트로 초기화
    	player2ReadyButton.setBackground(new Color(244, 246, 255)); // 초기 색상으로 되돌림
        // 오목판의 보드 상태 초기화
        boardLabel.clearBoard();  // BoardLabel에서 보드 초기화
        resetBoard();  // board 배열을 0으로 초기화

    }
    
    private void resetBoard() {
        // GameRoom의 board 배열 초기화
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                board[i][j] = 0;  // 보드의 모든 칸을 0으로 초기화
            }
        }
        giveupLabel.setVisible(false);
    }
